...
namespace Reciclaje_MVC.Controllers
{
    public class ReporteController : Controller
    {
        ...
        public ActionResult Reporte_Materiales()
        {
            return View();
        }
        //-------------------Metodos
        [HttpPost]
        [Route("Obtener_Reporte_Categoria_Materiales")]
        public ActionResult Obtener_Reporte_Categoria_Materiales(string fecha_inicio_reporte, string fecha_fin_reporte, string codigo_categoria, string codigo_producto, int tipo_reporte)

        {
            try
            {
                nReporte = new NReporte();
                var existe_usuario = new List<EReporte_Categoria_Materiales>();

                if (fecha_inicio_reporte != "" && fecha_fin_reporte != "")
                {
                    existe_usuario = nReporte.Obtener_Reporte_Categoria_Materiales(fecha_inicio_reporte, fecha_fin_reporte, codigo_categoria, codigo_producto, tipo_reporte);
                }
                //if (existe_usuario.Count == 0)
                //{
                //    HttpContext.Response.StatusCode = (int)HttpStatusCode.BadRequest;
        ...
        //-------------------Metodos
        [HttpPost]
        [Route("Exportar_Obtener_Reporte_Categoria_Materiales")]
        public ActionResult Exportar_Obtener_Reporte_Categoria_Materiales(string fecha_inicio_reporte, string fecha_fin_reporte, string codigo_categoria, string codigo_producto, int tipo_reporte)

        {
            try
            {
                ...
                if (fecha_inicio_reporte != "" && fecha_fin_reporte != "")
                {
                    var obj_pdf = nReporte.Obtener_Reporte_Categoria_Materiales(fecha_inicio_reporte, fecha_fin_reporte, codigo_categoria, codigo_producto, tipo_reporte);
                    if (obj_pdf != null)
                    {
                        var pdf_exportado = formato_pdf_exportado_Categoria_Materiales(obj_pdf);

                        if (tipo_reporte == 1)
                        {

                            nombre_pdfs = "Reporte_Todo_Categoria_Materiales" + DateTime.Now.ToString("ddMMyyy_hhmmss");
                        }
                        else
                        {
                            nombre_pdfs = "Reporte_Personalizado_Categoria_Materiales" + DateTime.Now.ToString("ddMMyyy_hhmmss");
                        }
        ...
        public string formato_pdf_exportado_Categoria_Materiales(List<EReporte_Categoria_Materiales> puntos_datos)
        {
            Document doc = new Document();
            ...
            PdfWriter writer = PdfWriter.GetInstance(doc, memoryStream);
            doc.AddAuthor("CJG - ECL");
            doc.AddTitle("Reporte General Materiales");
            doc.Open();
            ...
            //---------------Tercera tabla
            Tabla_Pdf = new PdfPTable(new float[] { 20f, 20f, 20f, 20f, 20f, 20f, 20f }) { WidthPercentage = 100 };

            Tabla_Pdf.AddCell(new PdfPCell(new Phrase("Codigo Interno".ToUpper(), letra_negrita_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
            Tabla_Pdf.AddCell(new PdfPCell(new Phrase("Codigo Interno Detallado".ToUpper(), letra_negrita_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
            Tabla_Pdf.AddCell(new PdfPCell(new Phrase("Fecha Registro".ToUpper().ToUpper(), letra_negrita_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
            Tabla_Pdf.AddCell(new PdfPCell(new Phrase("Usuario Canjeado".ToUpper().ToUpper(), letra_negrita_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
            Tabla_Pdf.AddCell(new PdfPCell(new Phrase("Categoria".ToUpper().ToUpper(), letra_negrita_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
            Tabla_Pdf.AddCell(new PdfPCell(new Phrase("Material".ToUpper().ToUpper(), letra_negrita_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
            Tabla_Pdf.AddCell(new PdfPCell(new Phrase("Cantidad".ToUpper(), letra_negrita_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });

            foreach (var item in puntos_datos)
            {
                Tabla_Pdf.AddCell(new PdfPCell(new Phrase(item.codigo_registro_reciclaje, letra_normal_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
                Tabla_Pdf.AddCell(new PdfPCell(new Phrase(item.codigo_registro_reciclaje_detalle, letra_normal_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
                Tabla_Pdf.AddCell(new PdfPCell(new Phrase(item.fecha_creacion_registro, letra_normal_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
                Tabla_Pdf.AddCell(new PdfPCell(new Phrase(item.nombre, letra_normal_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
                Tabla_Pdf.AddCell(new PdfPCell(new Phrase(item.descripcion_categoria, letra_normal_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
                Tabla_Pdf.AddCell(new PdfPCell(new Phrase(item.descripcion_producto, letra_normal_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
                Tabla_Pdf.AddCell(new PdfPCell(new Phrase(item.cantidad, letra_normal_7)) { HorizontalAlignment = Element.ALIGN_CENTER, VerticalAlignment = Element.ALIGN_CENTER });
            }
            doc.Add(Tabla_Pdf);
